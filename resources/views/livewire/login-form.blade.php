    ...
    <div class="min-h-screen py-6 flex flex-col justify-center sm:py-12">
        <div class="relative py-3 w-11/12 max-w-xl sm:mx-auto">
            <div class="relative p-8 bg-white shadow-sm sm:rounded-xl">
                <form method="POST" action="/login" wire:submit.prevent="submitForm">
                    @csrf
                     @if(session()->has('success'))
                            <div class="bg-green-500">{{session('success')}}</div>
                    @endif
                    <div class="mb-5 relative">
                        <input type="username" wire:model.defer="username" name="username" id="username" class="peer pt-8 border border-gray-200 focus:outline-none rounded-md focus:border-gray-500 focus:shadow-sm w-full p-3 h-16 placeholder-transparent" placeholder="shadowrunner245" autocomplete="off" />
                    ...
                    <div class="mb-5 relative">
                        <input type="password" wire:model.defer="password" name="password" id="password" class="peer pt-8 border border-gray-200 focus:outline-none rounded-md focus:border-gray-500 focus:shadow-sm w-full p-3 h-16 placeholder-transparent" placeholder="password" autocomplete="off" />
